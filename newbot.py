...
if not YOUR_BOT_TOKEN:
    raise ValueError("Токен бота не найден. Убедитесь, что переменная окружения YOUR_BOT_TOKEN установлена.")
...
def start_command(message):
    if is_admin(message.from_user.id):
        bot.send_message(message.chat.id, "Привет, админ! Используй /help чтобы увидеть список команд.")
    else:
